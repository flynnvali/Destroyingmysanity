...
public class PollutionHelper {
     ...
     * The change is broadcast to all clients (Avoid this by using the {@link com.petrolpark.destroy.core.pollution.Pollution#set set()} method instead).
     * @param level
     * @param pollutionType
     * @param value Will be set within the {@link com.petrolpark.destroy.core.pollution.Pollution.PollutionType bounds}.
     ...
     * The change is broadcast to all clients (Avoid this by using the {@link com.petrolpark.destroy.core.pollution.Pollution#change change()} method instead).
     * @param level
     * @param pollutionType
     * @param change Can be positive or negative; will be set within the {@link com.petrolpark.destroy.core.pollution.Pollution.PollutionType bounds}.
     * @return The actual value to which the level of pollution was set (0 if there was no Capability)
     */
    ...
    public static void pollute(Level level, BlockPos blockPos, float multiplier, int particleWeight, FluidStack ...fluidStacks) {
        ...
        for (FluidStack fluidStack : List.of(fluidStacks)) {
            ...
            if (particleWeight == 1 || level.getRandom().nextInt(particleWeight) == 0)
